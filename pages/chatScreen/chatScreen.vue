...
<script>
	...
	export default {
		...
		onReady() {
		  //this.messageData =	messageDate.messagelist;//设置默认的历史消息
		},
		onLoad(params) {
		...
		methods: {
			...
			handMessageSend(e){
				...
				//this.messageData = [].concat(JSON.parse(JSON.stringify(msgData.concat(e))));//拷贝数组,注意这行的拷贝方法
				this.messageData = msgData.concat(JSON.parse(JSON.stringify(e)));//拷贝数组,注意这行的拷贝方法
